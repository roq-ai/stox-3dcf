...
import { getScanners } from 'apiSdk/scanners';
import { ScannerInterface } from 'interfaces/scanner';
import { getAlerts } from 'apiSdk/alerts';
import { AlertInterface } from 'interfaces/alert';
import { getEquityStocks } from 'apiSdk/equity-stocks';
import { EquityStockInterface } from 'interfaces/equity-stock';
import { ScannerListPage as ShowCaseList } from 'pages/scanners';

function DashboardPage() {
  const { session } = useSession();

  const scannerStat = useSWR<PaginatedInterface<ScannerInterface>>(
    () => `/scanners`,
    () => getScanners({ limit: 1, offset: 0 }),
  );
  ...
  const equityStockStat = useSWR<PaginatedInterface<EquityStockInterface>>(
    () => `/equity-stocks`,
    () => getEquityStocks({ limit: 1, offset: 0 }),
  );

  const statResults = [
    {
      title: 'Scanners',
      result: scannerStat?.data?.totalCount,
      isLoading: scannerStat.isLoading,
    },
    {
      title: 'Alerts',
    ...
    {
      title: 'Equity Stocks',
      result: equityStockStat?.data?.totalCount,
      isLoading: equityStockStat.isLoading,
    },
  ];
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`scanners`} passHref legacyBehavior>
                  <Button
