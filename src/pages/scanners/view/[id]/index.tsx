...
import { getScannerById } from 'apiSdk/scanners';
import { ScannerInterface } from 'interfaces/scanner';
import { AlertListPage } from 'pages/alerts';

function ScannerViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<ScannerInterface>(
    () => (id ? `/scanners/${id}` : null),
    () =>
      getScannerById(id, {
        relations: ['company'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('scanner', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/scanners/edit/${id}`} passHref legacyBehavior>
                    <Button
