...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createCompany() {
    ...
    if (body?.equity_stock?.length > 0) {
      const create_equity_stock = body.equity_stock;
      body.equity_stock = {
        create: create_equity_stock,
      };
    } else {
      delete body.equity_stock;
    }
    if (body?.scanner?.length > 0) {
      const create_scanner = body.scanner;
      body.scanner = {
        create: create_scanner,
      };
    } else {
      delete body.scanner;
    }
